...
function otherMoreDetails(objid, cls) {
    //console.log(itisMobile + ' | ' + objid);
    //$('div[id^="divInfo"]').slideUp().removeAttr('style');
    //$('div[id^="dvArrow"]').slideUp();
    //$('.div.dveachOtherMore').find('span').css('transform', 'rotate(90deg)');

    //itisMobile ? $('#divInfo' + objid + '').attr('style', 'display:block; top:' + Number(pos.top + 55) + 'px; position: absolute; background: white; margin: 0 auto; width: 90%;') : $('#divInfo' + objid + '').slideDown();
    //$('#dvArrow' + objid + '').slideDown();
    //$('div.dveachOtherMore[lang="' + objid + '"]').find('span').css('transform', 'rotate(270deg)');
