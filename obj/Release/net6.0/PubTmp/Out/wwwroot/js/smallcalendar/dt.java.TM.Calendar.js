...
function doitDep() {
    if ($("#qLeaveNA").length > 0) {
        ...
        $("#qCabinTxt").autocomplete({
            ...
            minLength: 0,
            select: function (event, ui) {
                $("#qCabinTxt").val(ui.item.label); //.delay( 5000, function(){$( "body" ).trigger('click'); $( "#qCabinTxt" ).off('focus'); });
...
function findCombination() {
    ...
    $('input[id^="qFNACity"]').each(function () {
        ...
        if (ctyC == 1) {
            frmDestNA = ctyN; //$('#'+this.id+'').val();
            frmDestID = $('#' + this.id.replace('NA', 'ID') + '').val();
        }
        else {
            frmDestNA += '_' + ctyN; //$(this.id).val();
            frmDestID += ',' + $('#' + this.id.replace('NA', 'ID') + '').val();
        };
    });
    var findSite = '';
...
function buildFromCook(build) {
    ...
    if (build === 1) {
        $.each(cookieValues, function (i, e) {
            ...
            cokC += 1;
            cokC === cokL ? (Cookies.set('backCookieTMED', '', { expires: -1 }), dateByDest(cokR), $(window).scrollTop(ofset.top - 200)) : '';
        });
    }
}
...
//    //console.log("submitCompList.queryString = ");
//    //console.log(queryString);

//    var jsonQSTR = QueryStringToJSON(queryString)
//    $.ajax({
//        type: "POST",
//        url: "/latin/WS_Library.asmx/webservGetComponentList",
//        contentType: "application/json; charset=utf-8",
//        dataType: "json",
//        data: '{Q:' + JSON.stringify(queryString) + '}',
//        success: function (res) {
//            jsonCompList = eval("(" + res.d + ")");
//            //console.log("submitCompList.jsonCompList = ");
//            //console.log(jsonCompList);
//            submitToBP(jsonQSTR, jsonCompList);
//        },
//        error: function (xhr, desc, exceptionobj) {
