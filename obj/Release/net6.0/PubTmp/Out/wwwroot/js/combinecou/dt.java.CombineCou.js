...
$(document).ready(function () {
    ...
    $('a[id^="falsedotCou"]').click(function() {
...
function cutTitle(flatId,flatt){  
    ...
    if ($('#'+flatId).hasClass('falsechecked_1')){     
        title = title + "|" + flatt;      
        plcIds = plcIds + "," + flatId.match(isNumber);
        $('#IDplace').val(plcIds);
...
function initCitiesCheck(){
    ...
    for (x = 0; x <= nChecks.length - 1; x++) {
        if (nChecks[x] != '' && nChecks[x] != undefined) {
            $('#dotCou' + nChecks[x]).prop('checked', true);
        } else {
            return;    
		};
    }
        ...
        if (filterParts[i] != '' && filterParts[i] != undefined) {
            ...
            if (i == 2) {
                ...
                for (j = 0; j <= citiesParts.length - 2; j++) {
                    $('#dotCou' + citiesParts[j]).prop('checked', true);
                    $('#falsedotCou' + citiesParts[j]).addClass('falsechecked_1');                    
                }
            };
        }        
    }
...
function EnableUnckechOption() {
    ...
    var noCitiesOptions = $('input[type="checkbox"][id^="dotCou"]:checked').length;  
...
function UncheckOptions(filterOption) {
    ...
    if (filterOption == 'city') {
        if ($('#uncheckCities').hasClass('UncheckOptionDisabled'))
            return;      
        $('input[type="checkbox"][id^="dotCou"]:checked').each(function() {
            $(this).prop('checked', false);
        });
        $('a[id^="falsedotCou"]').each(function() {
            $(this).removeClass('falsechecked_1');
        });
        fChangeTitName('');
...
function fChangeTitName(title,visible){
    ...
    if (title != ''){ 
        ...
        if (partstrMsg == 1) {
            strinTitlinks = '<a href="/europe/' + strMsg[0].toLowerCase() + '/vacations">' + strMsg[0] + '</a>';
            partsTit = strMsg[0];
        }else{
            for (i in strMsg){
                if (i == partstrMsg - 1) {
                    strinTitlinks = strinTitlinks + ' | <a href="/europe/' + strMsg[i].toLowerCase() + '/vacations">' + strMsg[i] + '</a>';
...
function BuildPackets(msg) {    
    ...
    for (i in strMsg) {
        if (i < strMsg.length - 1) {                                            
            ...
            //Customer feedback			
            if (strMsgLn[7] != '0') {
                var placeNameP = placeName.replace(/\s/g, '_');
                var placeNameTK = placeNameP.split("_and_");
                var url = SiteName + "/" + strMsgLn[14].toLowerCase() + "/" + strMsgLn[1].replace(/\s/g, '_').toLowerCase() + "/feedback-" + strMsgLn[0] + "";
