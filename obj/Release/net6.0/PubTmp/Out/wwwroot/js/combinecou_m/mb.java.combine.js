...
function init() {
    ...
    $.ajax({
        type: "POST",
        url: SiteName + "/Api/Combine/GetAllCombinePackages",
...
function buildPacks(isFiltered, isPagination) {
    $('.dvPkList').html('');
    var msg = '';
    var nx = page * 10;
    var listH = "";
    ...
    if (isFiltered == undefined && isPagination == undefined) {
        ...
        if (isPagination == undefined) {
            $('.spNoPks').text(objPks.length + " Filtered");
        }
    }
    $.each(pkList, function () {
        msg = msg + "<div class='row pt-2 pb-2 border border-secondary border-top-0 border-right-0 border-left-0'><div class='col-4 pr-0'><img class='img-fluid' src='https://pictures.tripmasters.com" + this.imG_Path_URL.replace(" ", "_") + "' />" +
            "</div><div class='col-8 pl-0'><div class='col font14 blueText pr-0 font-weight-bold'>" + this.pdL_Title + "</div><div class='col'>";
        ...
        msg = msg + "</div><div class='col'><button type='button' class='btn btn-link font12 pl-0' data-target='#dvPkDesc" + this.pdlid + "' data-toggle='collapse' aria-expanded='false'>Show Itinerary Details</button></div>" +
            "<div class='col'><a href='" + SiteName + "/" + this.stR_PlaceTitle.toLowerCase() + "/" + this.pdL_Title.replace(/\s/g, '_').toLowerCase() + "/package-" + this.pdlid + "' class='btn btn-warning btn-block font14'>View It</a></div></div>";
        ...
        msg = msg + "</ul><div class='col-12 border border-secondary border-bottom-0 border-left-0 border-right-0 text-center'><a class='btn btn-link font12' href='" + SiteName + "/" + this.stR_PlaceTitle.toLowerCase() + "/" + this.pdL_Title.replace(/\s/g, '_').toLowerCase() + "/feedback-" + this.pdlid + "'>See out Customer feedbacks (" + this.noOfFeed + ")</a></div>";
        msg = msg + "</div></div></div>";
    });
    $('.dvPkList').append(msg);
}
...
function createPagination() {
    prevInt = 0;
    totPag = Math.ceil(objPks.length / 10);
...
function orderBy(obj) {
    ...
    switch (true) {
        case /radiochoice/i.test(obj.id):
            ...
            switch (no) {
                ...
                case "4":
                    ...
                    break;
                case "5":
                    sortList = sortList.sort(function (a, b) { return a.pdL_Title > b.pdL_Title ? 1 : a.pdL_Title < b.pdL_Title ? -1 : 0; });
                    var labeltxt = $("label[for=" + obj.id + "]").html();
                    $('.typeOrder').text(labeltxt);
                    break;
            }
        ...
        case /radioprice/i.test(obj.id):
            switch (obj.value) {
                case "0_999":
                    sortList = $.grep(sortList, function (price) { return (price.stP_Save.between(0, 999)); });
                    break;
                case "1000_1999":
                    sortList = $.grep(sortList, function (price) { return (price.stP_Save.between(1000, 1999)); });
                    break;
                case "2000_2999":
                    sortList = $.grep(sortList, function (price) { return (price.stP_Save.between(2000, 2999)); });
                    break;
                case "3000_3999":
                    sortList = $.grep(sortList, function (price) { return (price.stP_Save.between(3000, 3999)); });
                    break;
                case "4000_MAX":
                    sortList = $.grep(sortList, function (price) { return (price.stP_Save.between(4000, 9999)); });
            }
            break;
        case /radionight/i.test(obj.id):
            switch (obj.value) {
                case "0_7":
                    sortList = $.grep(sortList, function (nht) { return (nht.stP_NumOfNights.between(0, 7)); });
                    break;
                case "8_10":
                    sortList = $.grep(sortList, function (nht) { return (nht.stP_NumOfNights.between(8, 10)); });
                    break;
                case "11_14":
                    sortList = $.grep(sortList, function (nht) { return (nht.stP_NumOfNights.between(11, 14)); });
                    break;
                case "15_MAX":
                    sortList = $.grep(sortList, function (nht) { return (nht.stP_NumOfNights.between(15, 99)); });
                    break;
            }
    }
    objPks = sortList;
...
function applyFilter() {
    ...
    tmpInterval = priceno.split("_");
    if (tmpInterval[1] != "MAX") {
        pks = pks.filter(element => element.stP_Save >= tmpInterval[0] && element.stP_Save <= tmpInterval[1])
    }
    else {
        pks = pks.filter(element => element.stP_Save >= tmpInterval[0])
    }
    ...
    switch (sortno) {
        ...
        case "2":
            ...
            break;
        case "3":
            pks = pks.sort(function (a, b) { return a.stP_Save > b.stP_Save ? 1 : a.stP_Save < b.stP_Save ? -1 : 0; });
            break;
        case "4":
            pks = pks.sort(function (a, b) { return a.stP_Save < b.stP_Save ? 1 : a.stP_Save > b.stP_Save ? -1 : 0; });
            break;
