...
$(document).ready(function () {
	...
	$('.arrowDown').click(function () {
		openPackInfo($(this).attr("id"));
	});

	$('.aArrowHead').click(function () {
		...
		$('.dvotherInfo').each(function () {
			...
			idNum == pkID ? (
				$('#' + idDiv + '').is(':visible') == false ?
					(
						$('#divInfo' + pkID + '').slideDown(),
						$('#arrowInfo' + pkID + '').show(),
						$('#' + pkID + '').css('transform', 'rotate(-90deg)'),
						relPackCallt4(pkID)
					) : (
						$('#divInfo' + idNum + '').slideUp(),
						$('#arrowInfo' + idNum + '').hide(),
						$('#' + idNum + '').css('transform', 'rotate(90deg)')
					)
			) : (
				$('#divInfo' + idNum + '').slideUp(),
				$('#arrowInfo' + idNum + '').hide(),
	...
	$('p[id^="close"]').click(function () {
		var pkID = this.id.match(isNumber)
		$('#divInfo' + pkID + '').slideUp();
		$('#arrowInfo' + pkID + '').hide();
		$('#' + pkID + '').css('transform', 'rotate(90deg)');
	});
	...
	$('.moreButtonT4').click(function () {
		$('#toHide').is(':visible') === false ?
			($(".toHide").slideDown(), $('.moreButtonT4').html('Close More Highlights & Attractions')) : ($(".toHide").slideUp(), $('.moreButtonT4').html('More Highlights & Attractions'));
	});
...
function openPackInfo(pkId) {
	var idPak = pkId.match(isNumber);
	$('.suggInfo').each(function () {
		var divId = $(this).attr('id');
		var numId = divId.match(isNumber);
		Number(idPak) == Number(numId) ? (
			$('#' + divId + '').is(':visible') == false ?
				(
					$('#' + divId + '').slideDown(),
					$('#arrdown' + numId + '').css('transform', 'rotate(-90deg)'),
					relPackCallt4(idPak)
				) : (
					$('#' + divId + '').slideUp(),
					$('#arrdown' + numId + '').css('transform', 'rotate(90deg)')
	...
	$('.dvotherInfo').each(function(){
			...
			idNum == objid ? ( 
				$('#'+idDiv+'').is(':visible') == false ? (
					...
					$('div.dveachOtherMore[data-go-to="'+ idNum +'"]').find('span').css('transform','rotate(270deg)')
										
					) : (
					
					$('#'+idDiv+'').slideUp(),
					$('#dvArrow'+idNum+'').slideUp(),
						$('div.dveachOtherMore[data-go-to="'+ idNum +'"]').find('span').css('transform','rotate(90deg)')
					)
...
function CarrouPicsBuild(phoN){
	...
	if (picTotal <6){var leftPadd = 18*(6-picTotal);
	}else{leftPadd= 0;}	
