...
$(document).ready(function() {
	...
	//		$('#'+this.id.replace('B','A')+'').val(this.value);
	//	}else{
...
// *****************************************************
	...
	$('span[id*="aboutSP"]').click(function (e) {
		...
		$('#'+popID+'').attr('style','padding:5px; position:absolute; width:800px; display:none; z-index:9998; left:'+ popL + 'px; top:'+ popT +'px'); // popW/2- $(popID).width()/2); 
		$('#'+popID+'').fadeIn(2000);
	...
	buildPics();
	readCookVal();
	if (fxDates == '0') { dateByDest2426(); } //duplicate dateByDest in smallcalendar/dt.java.tm.calendar.js called in Layout
			...
			if(roomAndpax == undefined){			  
				...
				if (jsRoom > 1){
					...
					for (r=2;r<=jsRoom;r++){
						...
						$('#Room'+r+'_iAdults').val(jsAdult);//Room2_iAdults
...
function newDest() {
	...
	/*$("#BsDepCity").click(function() {
		citySelID = "BsDepCity";			
		if (IsMobileDevice()){$('#BsDepCity').val('');}else{$("#BsDepCity").select()};
	});*/
...
function doit() {
    ...
    //		return row.plC_Title + " - " + row.plC_Code;
    //	},
    ...
    //	}
    //});
	/*$("#BsDepCity").autocomplete(citiesAll, {
		minChars: 3,
		width: 310,
		matchContains: "word",
		autoFill: false,
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (formatCurrency(infoLN[4]) != '$0') {
				if (infoLN[3] == 1) { //6 || infoLN[9] == 6 || infoLN[9] == 26 || infoLN[9] == 27){
    ...
    $.ajax({
        ...
        success: function(data) {
			...
			if(ntsSet == 1){
				...
				samPrice = samPrice + '<span id="intsBubble'+itinID+'">';// style="border-bottom:dotted 2px #F90; cursor:pointer;">'
			}
		...
		if (strRel[0] != packID){					 
			...
			if(eachC == 0){margT = '10px'}else{margT = '5px'};
				...
				if (strRel[2] != 9999 && strRel[2] != ''){
					if (strRel[4] == 1){ // || strRel[4] == 6 || strRel[4] == 26){
...
function aboutBUT(){
	var popID;
	var objGO;
	//$('.jxCoInf').popupWindow({centerBrowser:1, height:600, width:750, resizable:1, scrollbars:1});
	for (i=0;i<=totCities;i++){
		$('#aboutPL'+i+'').click(function (e) {
			...
			$('#mask27').fadeIn(1000);
			$('#mask27').fadeTo("slow",0.8);
			$('#'+popID+'').attr('style','padding:5px; position:absolute; width:800px; margin-left:-400px; display:none; z-index:9998; left:50%; top:'+posT+'px;'); // popW/2- $(popID).width()/2); 
			$('#'+popID+'').fadeIn(2000);
			scrollToMorePop(popID);
...
function winOpenCMS(jhref) {
    //var jhref = '/europe/cms/' + id + '/Web_Content.aspx?cms&wh=0&wf=0';
    centerWindow(jhref);
    return false;
};
...
function popError(obj,messg){
	...
	/*
	var objPos = $('#'+obj+'').position();//document.getElementById(obj);
	$('#dvErrMess').html('<img src="/europe/images/Symbols-Warning-icon-32.gif" width="32" height="32" align="absmiddle" style="padding-top:5px;" /> '+mess+'');
	var popL =objPos.left - 5; //objPos[1] //- 5; 
	var popT = objPos.top - 80; //objPos[0] //- 80
	//$('#'+obj+'').focus();
	//$('#'+obj+'').select();
