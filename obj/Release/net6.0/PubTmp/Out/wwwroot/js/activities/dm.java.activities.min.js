function deleteFilterItem(n){filterResults=filterResults.filter(function(t){return t.id!==n})}function chooseType(n){const i=$("#"+n);i.toggleClass("selected");i.addClass("disabled");const u=n.replace("false",""),t=$('input[type="checkbox"][id="'+u+'"]');if(t.is(":checked"))t.prop("checked",!1),deleteFilterItem(n);else{t.prop("checked",!0);const i=t.val();getAmountActivities(i);filterResults.push({name:$("#"+n+" > span:nth-child(1)").text(),id:n})}typeFilter="";let r=0;$('input[type="checkbox"][id^="dotType"]:checked').each(function(){typeFilter+=$(this).val()+"T";r++});r===0&&(typeFilter=$("#typeFilter").val());GetCitySSData();setTimeout(()=>i.removeClass("disabled"),1e3)}function chooseFavorite(){$("#falseshowFav").hasClass("falsechecked_1")?($("#falseshowFav").removeClass("falsechecked_1"),$("#showFav").prop("checked",!1),deleteFilterItem("falseshowFav")):($("#falseshowFav").addClass("falsechecked_1"),$("#showFav").prop("checked",!0),filterResults.push({name:'Show only <b style="color: #090;">Favorites✓<\/b>',id:"falseshowFav"}));GetCitySSData()}function getAmountActivities(n){var i=$("#SSIds").val(),t={};t.url=SiteName+"/Api/CitySSData";t.type="POST";t.contentType="application/json; charset=utf-8";t.data='{"SSFilter":"'+n+'T|0||1", "Ids":"'+i+'"}';t.dataType="text";t.success=function(t){var i=t.substr(1,t.length-2).split("@");const r=i[1];$("#falsedotType"+n+" > span:nth-child(2)").text("("+r+")")};t.error=function(n){console.error(n.responseText)};$.ajax(t)}function resetFilter(){$(".filter-result").addClass("is-hidden");filterResults=[];$('label[id*="sortBy"]').removeClass("active");$("#sortBy1").addClass("active");$('input[type="radio"][id="orderAll"]').prop("checked",!0);$("#tourName").val("");$('input[type="checkbox"][id="showFav"]').prop("checked",!1);$("#falseshowFav").removeClass("falsechecked_1");$('li[id^="falsedotType"]').removeClass();$('input[type="checkbox"][id^="dotType"]').prop("checked",!1);typeFilter=$("#typeFilter").val();GetCitySSData()}function submitInputName(){var n=$("#tourName").val();if(n==""){messg="<ol><li>Please insert an activity name!<\/li><\/ol><span><\/span>";popError("tourName",messg);return}filterResults.push({name:"Tours name contains: <b>"+n+"<\/b>",id:"tourName"});$(".filter-result").removeClass("is-hidden");GetCitySSData()}function setCheckboxOnly(n){n=="TypeFavorite"?$("#falseshowFav").trigger("click"):($('a[id^="falsedot"]').removeClass().addClass("falsecheck_1"),$('input[id^="dot"]').prop("checked",!1),$("#falsedot"+n).addClass("falsechecked_1"),$("#dot"+n).prop("checked",!0),GetCitySSData())}function buildFilterResults(){const n=filterResults.map(function({name:t,id:n}){return`<li>
                    <span>${t}</span>
                    <button type="button" onclick="closeFilterResultItem('${n}')">✖</button>
                </li>`});$("#filterResultsList").html(n)}function closeFilterResultItem(n){switch(n){case"tourName":deleteFilterItem(n);$("#"+n).val("");GetCitySSData();break;case"falseshowFav":deleteFilterItem(n);$("#"+n).removeClass("falsechecked_1");$("#showFav").prop("checked",!1);GetCitySSData();break;default:chooseType(n)}}function BuildFilterExpression(){var n="",t,i;return n=$("#showFav").is(":checked")?"1":"0",t=$("#tourName").val(),i=$('input[name="orderValue"]:checked').val(),typeFilter==""&&(typeFilter="999999T"),typeFilter+"|"+n+"|"+t+"|"+i}function GetCitySSData(){var i,t,n;desk.page=1;mob.page=1;$("#showMoreActivitiesMob").hide();$("#showMoreActivities").hide();$(desk.element).html("");$(mob.element).html("");$("#dvWait").html('<img src="https://pictures.tripmasters.com/siteassets/d/ajax-loader.gif" width="50" height="50" /><\/br><\/br><div style="margin-let:10px;color:navy;">Loading ...<\/div>');$("#dvWait").show();filterResults.length?(buildFilterResults(),$(".filter-result").removeClass("is-hidden")):$(".filter-result").addClass("is-hidden");i=BuildFilterExpression();t=$("#SSIds").val();t.trim()!=""?(n={},n.url=SiteName+"/Api/CitySSData",n.type="POST",n.contentType="application/json; charset=utf-8",n.data='{"SSFilter":"'+i+'", "Ids":"'+t+'"}',n.dataType="text",n.success=function(n){msg=n.substr(1,n.length-2);var t=msg.split("@");$("#SSIdsPages").val(t[0]);isFirstLoad&&($(".spanTotalSS").html(t[1]),isFirstLoad=!1);$("#totalFilterResults").html(t[1]+" total");total=t[1];OpenPage(desk.element,0,desk.numOfItems);OpenPage(mob.element,0,mob.numOfItems)},n.error=function(n){$(desk.element).html(n.responseText);$(mob.element).html(n.responseText);$("#dvWait").html("");$("#dvWait").hide()},$.ajax(n)):($(desk.element).html('<div class="activities__no-result">0 activities found. Please try other combination.<\/div>'),$(mob.element).html('<div class="activities__no-result">0 activities found. Please try other combination.<\/div>'),$("#dvWait").html(""),$("#dvWait").hide())}function OpenPage(n,t,i){var r;$("#showMoreActivities").html('<img src="https://pictures.tripmasters.com/siteassets/d/ajax-loader.gif" width="18" height="15" />');$("#showMoreActivitiesMob").html('<img src="https://pictures.tripmasters.com/siteassets/d/ajax-loader.gif" width="18" height="15" />');var u=BuildFilterExpression(),f=u.split("|"),e=$("#SSIdsPages").val(),o=e.split("|").join(",").replaceAll(","," ").trim().split(" ");const s=o.slice(t,i).join(",");total>0?(r={},r.url=SiteName+"/Api/CityActivities",r.type="POST",r.contentType="application/json; charset=utf-8",r.data='{"SSFilter":"'+s+'", "Ids":"'+f[3]+'"}',r.dataType="json",r.success=function(t){let i="";jQuery.each(t,function(){i+=`<li>
