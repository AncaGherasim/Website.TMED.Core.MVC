...
$(document).ready(function () {
    ...
    $(document.body).on('click', '.iconUp', function () {
        ...
        $(inpDn).animate({ 'opacity': '0', 'margin-top': '100px' }, 300); 
        $(inpUp).animate({ 'margin-top': '0px', 'opacity': '1' }, 300); 
    ...
    $(document.body).on('click', '.iconDwn', function () {
        ...
        $(inpUp).animate({ 'opacity': '0', 'margin-top': '-30px' }, 300); 
        $(inpDn).animate({ 'margin-top': '0px', 'opacity': '1' }, 300);
    ...
    backCook = Cookies.get('TMEDpkbyoBack'); 
...
function hidedialogCont() {
    ...
    haveCook = 0;
    var qPxRoomString = qRooms + ', ' + totpax + ' Travelers';
...
function doitDeparture(ctyNa) {
    ...
    $('#' + ctyNa + '').autocomplete({
        ...
        select: function (event, ui) {
            ...
            $('#' + ctyNa + '').attr('value', ui.item.value);
        ...
        (ctyNa == 'xtxtLeavingFrom') ? ($('#xtxtLeavingFrom').removeClass('errorClass'), $('#xtxtLeavingFrom').attr('value', ''), $('#xIDLeavingFrom').val('-1')) : ''
...
function addnewcity(obj) {
    ...
    var frm = frmto[0];
    var to = frmto[1];
    if (to > 12) { alert('For best results, no more than 12 cities is allowed. Thanks.'); return false; };
...
function submitToBook() {
    ...
    if (withFlight == true) {
        if ($('#xIDLeavingFrom').val() == -1) {
            ...
            var errMess = 'Select a valid city or airport';
            errorAlert('xtxtLeavingFrom', errMess);
            return false;
        };
        ...
        if ($('#xIDReturningTo').val() == -1) {
            ...
            var errMess = 'Select a valid city or airport';
            errorAlert('xtxtReturningTo', errMess);
            return false;
        };
        ...
        if (cab == '') {
            ...
            var errMess = 'Select a cabin type';
            errorAlert('xCabinTxt', errMess);
            return false;
        };
    };
    //Date validation
    ...
    if (idate == 'mm/dd/yyyy' || idate == '') {
        ...
        var errMess = 'Date is not valid';
        errorAlert('xtxtBYArriving', errMess);
        return false;
    };
    ...
    for (i = 1; i <= totCity; i++) {
        if ($('#xsCityID' + i + '').val() == -1) {
            var clsClk = "'xsCityNA" + i + "'";
            var errMess = 'Select a valid city';
            errorAlert('xsCityNA' + i, errMess);
            return false;
        };
    };
    ...
    var cookString = ''
    cookString = JSON.stringify($('#getNewItin').serializeObject());
