...
function submitFormT4(frmNA) {
    ...
    setItemF("localStorageTMEDBYO", stringQuery, 1);
    var stringForm = '';
...
function buildTransportationFrom() {
    var cookSaved = JSON.parse(getItemF("localStorageTMEDBYO"));
...
function logErrorToCloudWatch(errorMessage, jscity) {
    ...
    return fetch(SiteName + '/Api/errorcalendarlogs', {
        method: 'POST',
        headers: {
...
function buildTransportationError(err) {
    $('.goToStartAgain, #btnEdit, #btnBack, #btnBackDsk').click(function () { startAgain(this) });
    var cookieValues = JSON.parse(getItemF("localStorageTMEDBYO"));
    var cokC = 0;
    ...
    jQuery.each(cookieValues, function (i, e) {
        ...
        cokC += 1;
        cokC === cokL ? (
            removeItemF("localStorageTMEDBYO"),
            $('#sptrancities').html(ctys), $('#perrorCode').html(err), $('.dvWaitGlobe').hide(), $('.transportation__wrapper').hide(), $('#dvTranspError').show()) : ''
    });
};
...
function cookieCheck() {
    selectRoomPax('1|2');
    backCookie = getItemF("localStorageTMEDBYO");
    backCookie !== null ? (haveCook = 1, buildFromCook(1), doitArr()) : (haveCook = 0, dateByDest(), doitArr(), getHomeTownAirport());
}
function buildFromCook(build) {
    ...
    if (build === 1) {
        $.each(cookieValues, function (i, e) {
            i === 'qWair' ? (e === 'True' ? $('#updateFlightToTrue').trigger('click') : $('#updateFlightToFalse').trigger('click')) : '';
