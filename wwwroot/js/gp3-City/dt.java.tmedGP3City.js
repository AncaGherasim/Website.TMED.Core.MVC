...
$(document).ready(function () {
    ...
    $('.moreButton').click(function () {
        var thisID = this.id;
        $(".li-Hide").length > 0 ?
            ($(".li-Hide").slideDown().toggleClass('li-Hide li-Show').removeAttr('style'), $("#" + thisID + "").text($("#" + thisID + "").text().replace("See", "Close"))) :
            ($(".li-Show").slideUp().toggleClass('li-Show li-Hide').removeAttr('style'), $("#" + thisID + "").text($("#" + thisID + "").text().replace("Close", "See")));
    });

    $('.moreButtonExpl').click(function () {
        $('.dvEachHideExpl').is(':visible') === false ?
            ($(this).html('Close All ' + moreExplore), $('.dvEachHideExpl').slideDown(), $('.dvEachHideExpl').addClass('ulVisible'))
            : ($(this).html('See All ' + moreExplore), $('.dvEachHideExpl').slideUp(), $('.dvEachHideExpl').removeClass('ulVisible'));
    });
    ...
    //Function that get the number of featured packages by place id 
    if (viewUsed == 'GP3_Area' || viewUsed == 'GP3_City') {
        const targetElement = document.querySelector('.buildButton');
...
function getNoOfPacksFeaturedItin() {
    ...
    fetch(SiteName + "/Api/GetNoOfPacksFeaturedItin/" + parseInt(plcId))
        .then(response => response.json())
        .then(data => {
            if (data[0] != 0) {
                $('.btn-byo').css('display', 'inline-block');
                $('.noPacks').html(data[0].noOfPacks)
            }
        })
        .catch(error => {
...
function findPacks(formID) {
    ...
    if (idID == undefined) {
        ...
        $('#allID').val(idID);
        $('#allNA').val(idNA);
        window.location.href = SiteName + '/' + idNA.toLowerCase().replace(" ","_") + '/find-packages';
    }
}
...
function triptaketo(cotogo) {
    ...
    var plid = goparts[1];
    var plna = goparts[0];
    window.location = "/europe/" + plna.toLowerCase().replace(" ","_") + "/trips_taken_by_travelers"
}
