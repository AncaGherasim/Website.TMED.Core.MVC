...
$(document).ready(function () {
	...
	//console.log($("." + parentSec + " .li-Hide").length);
	...
	$.ajax({
		...
		success: function (data) {
			objListReviews = data;
			//console.log(objListReviews);
			var ulListReviews = '<ul class="ul-Flex">';
			var commC = 0;
			for (i = 0; i <= objListReviews.length - 1; i++) {
				ulListReviews = ulListReviews + '<li class="liBkg"><div class="div-ContainerFlex"><a href="' + SiteName + '/' + placeNA.replace(/ /g, "_").toLowerCase() + "/" + objListReviews[i].PDL_Title.replace(/ /g, "_").toLowerCase() + "/package-" + objListReviews[i].PDLID + '"><p class="review_title">' + objListReviews[i].PDL_Title + '</p></a>';
				...
				var bookingDate = new Date(objListReviews[i].BookingDate);
