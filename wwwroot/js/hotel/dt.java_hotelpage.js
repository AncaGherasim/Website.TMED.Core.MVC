...
$(document).ready(function () {
    ...
    getHotelPlacePackagesIdeas(STR_PlaceID);
    getHotelInfoDetails(hotelID);
});
    ...
    try {
        ...
        options.success = function (data) {
            ...
            if (objListPrices.length > 0) {
                var listPacks = '<h2 class="suggested-section__title">Suggested itineraries with ' + STR_PlaceTitle + '</h2>';
                listPacks += '<ul class="suggested-section__list">';
                objListPrices.slice(0, 4).forEach((pr) => {
                    var pathURL = pr.IMG_Path_URL == null ? "/siteassets/d/no-image.jpg" : pr.IMG_Path_URL.toLowerCase();
                    listPacks += '<li class="suggested-section__item">';
                    listPacks += '<div class="suggested-section__wrapper">';
                    listPacks += '<a href="' + SiteName + '/' + pr.CountryName.replace(/ /g, '_').toLowerCase() + '/' + pr.PDL_Title.replace(/ /g, '_').toLowerCase() + '/package-' + pr.PDLID + '">';
                    listPacks += '<img src="https://pictures.tripmasters.com' + pathURL + '" alt="' + pr.PDL_Title + '" />';
                    listPacks += '<div>';
                    listPacks += '<h4>' + pr.PDL_Title + '</h4>';
                    listPacks += '</div>';
                    listPacks += '</a>';
                    listPacks += '</div>';
                    listPacks += '<div class="suggested-section__info">';
                    listPacks += '<p class="suggested-section__nights">';
                    if (pr.STP_Save != 9999) {
                        listPacks += pr.PDL_Duration + ' nights from <span>' + formatCurrency(pr.STP_Save, 0) + '</span>';
                    }
                    else {
                        listPacks += pr.PDL_Duration + ' nights itinerary <span>&nbsp;</span>';
                    }
                    listPacks += '</p>';
                    listPacks += '<button class="suggested-section__details" type="button" onclick="otherMoreDetails(\'' + pr.PDLID + '\',1),toggleClassMoreDetails(' + pr.PDLID + ')">more details <span>&#8250;</span></button>';
                    listPacks += '</div>';
                    listPacks += '<div class="ft-modal is-hidden" id="fti-' + pr.PDLID + '-modal">';
                    listPacks += '<div class="ft-modal-content">';
                    listPacks += '<div class="ft-modal-header">';
                    listPacks += '<h3>' + pr.PDL_Title + '</h3>';
                    listPacks += '</div>'
                    listPacks += '<div class="ft-modal-body">';
                    listPacks += '<p>' + pr.SPD_Description + '</p>';
                    listPacks += '<div class="divFlexModal">';
                    listPacks += '<div>';
                    listPacks += '<p>Includes:</p>';
                    listPacks += '<ul>';
                    var inclIdeasList = pr.PDL_Content.split("\r\n");
                    inclIdeasList.forEach(function (inc) {
                        listPacks += '<li>' + inc + '</li>';
                    });
                    listPacks += '</ul>';
                    listPacks += '</div>';
                    ...
                    if (pr.STP_Save != 9999) {
                        listPacks += '<p>';
                        listPacks += pr.PDL_Duration + ' nights from <span style="color:#f60;">' + formatCurrency(pr.STP_Save, 0) + '*</span>';
                        listPacks += '</p>';
                    }
                    else {
                        listPacks += '<p>' + pr.PDL_Duration + ' nights</p>';
                    }
                    listPacks += '<a class="aModal" href="' + SiteName + '/' + pr.CountryName.replace(/ /g, '_').toLowerCase() + '/' + pr.PDL_Title.replace(/ /g, '_').toLowerCase() + '/package-' + pr.PDLID + '">';
                    listPacks += '<span class="customBtn">Customize it</span>';
                    listPacks += '</a>';
                    listPacks += '</article>';
                    listPacks += '</div>';
                    listPacks += '</div>';
                    listPacks += '<div class="dvRelPack" id="dvRel' + pr.PDLID + '">';
                    listPacks += '<img src="https://pictures.tripmasters.com/siteassets/d/ajax-indicator.gif"/>';
                    listPacks += '</div>';
                    listPacks += '<form name="frm' + pr.PDLID + '" id="frm' + pr.PDLID + '" method="post">';
                    listPacks += '<div class="dvMorChoice" id="dvCom' + pr.PDLID + '">';
                    listPacks += '<img src="https://pictures.tripmasters.com/siteassets/d/ajax-indicator.gif" />';
                    listPacks += '</div>';
                    listPacks += '<div class="divLast">';
                    listPacks += '<a onclick="findPackst4(\'frm' + pr.PDLID + '\'); return false">';
                    listPacks += '&gt;&gt;';
                    listPacks += 'Find similar package';
                    listPacks += '</a>';
                    listPacks += '</a>';
                    listPacks += '</div>';
                    listPacks += '<hr class="spdotLine" />';
                    listPacks += '<input type="hidden" name="allIDsdp" id="allIDsdp" value=""/>';
                    listPacks += '<input type="hidden" name="allID" id="Hidden1" value="" />';
                    listPacks += '<input type="hidden" name="allNA" id="Hidden2" value="" />';
                    listPacks += '</form>';
                    listPacks += '</div>';
                    listPacks += '<div class="ft-modal-footer">';
                    listPacks += '<button id="aClose' + pr.PDLID + '" class="ft-modal-close" onclick="toggleClassMoreDetails(' + pr.PDLID + ')">[&#10006;] Close </button>';
                    listPacks += '</div>';
                    listPacks += '</div>';
                    listPacks += '</div>';
                    listPacks += '</li>';
                ...
                $('.suggested-section').append(listPacks);
            }
        };
...
function getHotelInfoDetails(hotelID) {
    $.ajax({
        type: "POST",
        url: SiteName + "/Api/HotelInfoDetails",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        ...
        success: function (data) {
            ...
            if (objListPrices.length > 0) {
                var HotelStyle = "";
                var NoRooms = "";
                if (objListPrices[0].GIEX_ExpSpecialCheckin != null) {
                    var expSpecialCheckin = '<div class="info-section">';
                    expSpecialCheckin += '<h2 class="info-section__title">special check-in instructions</h2>';
                    expSpecialCheckin += '<p class="info-section__text">' + objListPrices[0].GIEX_ExpSpecialCheckin + '</p>';
                    expSpecialCheckin += '</div>';

                    $('#GIATA_ExpediaExt').append(expSpecialCheckin);
                }
                if (objListPrices[0].GIEX_MandatoryFees != null || objListPrices[0].GIEX_OptionalFees != null) {
                    var expSpecialCheckin = '<div class="info-section">';
                    expSpecialCheckin += '<h2 class="info-section__title">fees</h2>';
                    if (objListPrices[0].GIEX_MandatoryFees != null) {
                        expSpecialCheckin += '<div class="info-section__text">' + objListPrices[0].GIEX_MandatoryFees + '</div>';
                    }
                    if (objListPrices[0].GIEX_OptionalFees != null) {
                        expSpecialCheckin += '<div class="info-section__text">' + objListPrices[0].GIEX_OptionalFees + '</div>';
                    }
                    $('#GIATA_ExpediaExt').append(expSpecialCheckin);
                }
                if (objListPrices[0].GIEX_Renovations != null) {
                    var expSpecialCheckin = '<div class="info-section">';
                    expSpecialCheckin += '<h2 class="info-section__title">Renovations and closures</h2>';
                    expSpecialCheckin += '<p class="info-section__text">' + objListPrices[0].GIEX_Renovations + '</p>';
                    expSpecialCheckin += '</div>';

                    $('#GIATA_ExpediaExt').append(expSpecialCheckin);
                }

                if ((objListPrices[0].GIPH_TNContentSource.endsWith('tournet content') || objListPrices[0].GIPH_TNUseTournetContent == true) && objListPrices[0].GIPH_TNRooms > 0) {
                    NoRooms = (objListPrices[0].GIPH_TNRooms == null) ? "-" : objListPrices[0].GIPH_TNRooms;
                }
                else {
                    NoRooms = (objListPrices[0].HotelNoRooms == null) ? "-" : objListPrices[0].HotelNoRooms;
                }
                $('#NoRooms').html(NoRooms);

                if (objListPrices[0].GIPH_TNContentSource.endsWith('tournet content') || objListPrices[0].GIPH_TNUseTournetContent == true) {
                    let uniqueArray = [...new Set(objListPrices.map(x => x.HotelStyleManual))];
                    uniqueArray.forEach((style) => {
                        HotelStyle = HotelStyle + (HotelStyle == "" ? "" : ", ") + (style == null ? "" : style);
                    })
                }
                else {
                    let uniqueArray = [...new Set(objListPrices.map(x => x.HotelStyle))];
                    uniqueArray.forEach((style) => {
                        HotelStyle = HotelStyle + (HotelStyle == "" ? "" : ", ") + (style == null ? "" : style);
                    })
                }
                $('#HotelStyle').html(HotelStyle == "" ? "-" : HotelStyle);
            }
        },
        error: function (xhr, desc, exceptionobj) {
            console.log("Call getHotelPlacePackagesIdeas error: " + xhr.responseText);
