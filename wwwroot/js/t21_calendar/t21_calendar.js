...
function readCookVal() {
	...
	if (backNull != null) {
		...
		if (backCook.Pkgid != packagesID) {
			...
			if (flyAdd === 'False') {
				$("#updateFlightToFalse").prop("checked", true);
				$('#AirParam').slideUp()
				$('#dateLabel').text('Check-in Date on your first city');
				$('#dateWrapper').removeClass('active');
				$('#addFlight').val('False');
...
function builFixDatediv(dates, dvObj) {
	...
	var postField;
	var m = '';
	var dtsDV = '<p class="pSelectDate">Select a date</p>';
	var fecha = dates.split(',');
	var dtct;

	var isTMGuided = $('#isTMGuided').val();
	var maxAllowedDate = null;
	if (isTMGuided == 1) {
		maxAllowedDate = new Date();
		maxAllowedDate.setMonth(maxAllowedDate.getMonth() + 11);
	}

	for (i = 0; i < fecha.length; i++) {
		if (maxAllowedDate) {
			var currentDateObj = stringToDate(fecha[i], "mm/dd/yyyy", "/");
			if (currentDateObj > maxAllowedDate) {
		...
		if (m != dateFormat(fecha[i], "mmm")) {
			...
			dtct = 0;
			if (i == 0) {
				dtsDV = dtsDV + '<div class="dvMontDate">';
			}
			else {
				dtsDV = dtsDV + '</div></div><div class="dvMontDate">';
			};
			dtsDV = dtsDV + '<p class="pMMYYYY">' + m + ' ' + dateFormat(fecha[i], "yyyy") + ':</p><div class="dvEachDate">';
		};
