...
$(document).ready(function () {
	...
	try {
		...
		options.url = SiteName + "/Api/Hotels/HotelsOnPlaceMap",
			options.type = "GET";
		...
		options.error = function (xhr, desc, exceptionobj) {
			console.log("Call HotelsOnPlaceMap error: " + xhr.responseText);
		};
		$.ajax(options);
	}
	catch (error) { console.error('HotelsOnPlaceMap error:', error); }
});
function initpoi() {
    $.ajax({
        type: "POST",
...
function javaFilter(){
	...
	if (jsRevC > 0) {
		...
		objHotels = grepRev(objHotels, jsRev)
	};
	// *** Point Of Interest  *** //
...
function buildHotel() {
	...
	jQuery.each(objHotels, function (objHotels) {
		...
		var icoImg;
		//console.log(imgName);
		//console.log(/Stars|Lodge|Cruise/g.test(imgName));
...
function bubbleHotelInfo(id) {
	...
	try {
		options.traditional = true;
		options.url = SiteName + "/Api/Hotels/HotelMapAddress",
			options.type = "GET";
		options.data = gethotelsparams,
			options.dataType = "json";
		options.cache = false;
		options.success = function (data) {
			jQuery.each(oneHotel, function (oneHotel) {
				...
				if (imG_Url != '') {
					message = message + '<div class="info-hotel__image"><img src="https://pictures.tripmasters.com' + imG_Url.toLowerCase() + '"/></div>';
				};
				message = message + '<div>'
				...
				if (Array.isArray(data) && data.length > 0) {
					if (
						(data[0].giph_tncontentsource.endsWith('tournet content') || data[0].giph_tnusetournetcontent) &&
						safeTrim(data[0].giph_addressline1) == '' &&
						safeTrim(data[0].giph_addressline2) == '' &&
						safeTrim(data[0].giph_addressline3) == ''
					) {
						HotelAddress = data[0].pty_address == null ? '' : data[0].pty_address;
					} else {
						HotelAddress =
							safeTrim(data[0].giph_addressline1) +
							(data[0].giph_addressline2 == null ? '' : ', ' + safeTrim(data[0].giph_addressline2)) +
							(data[0].giph_addressline3 == null ? '' : ', ' + safeTrim(data[0].giph_addressline3)) +
							(data[0].giph_addressline4 == null ? '' : ', ' + safeTrim(data[0].giph_addressline4)) +
							(data[0].giph_addressline5 == null ? '' : ', ' + safeTrim(data[0].giph_addressline5)) +
							(data[0].giph_addressline6 == null ? '' : ', ' + safeTrim(data[0].giph_addressline6));
					}
					if (
						data[0].giph_tncontentsource == 'giata/giata content' &&
						!data[0].giph_tnusetournetcontent
					) {
						HotelDescription =
							(data[0].ghgt_text100 == null ? '' : data[0].ghgt_text100) +
							'</br></br>' +
							(data[0].ghgt_text101 == null ? '' : data[0].ghgt_text101);
					} else if (data[0].giph_tntournetcontent) {
						try {
							const parser = new DOMParser();
							const xmlDoc = parser.parseFromString(data[0].giph_tntournetcontent, 'text/xml');
							const propertyNode = xmlDoc.querySelector('Property');
							if (propertyNode) {
								const descriptionNode = propertyNode.querySelector('Description');
								if (descriptionNode) {
									HotelDescription = descriptionNode.textContent;
								}
							}
						} catch (e) {
							HotelDescription = '';
				...
				message = message + imgSrc + '<p>' + HotelAddress + '</p>';
				var scrST = this.ghs_finalscore;
				var expScore = expediaNoRange(this.ghs_finalscore);
				switch (expScore) {
					case 'EX':
				...
				message = message + '<div>' +
					...
					NoOfReviews +
					'</div>';
				message = message + '<br style="clear:both" />';
				message = message + '</div>'

				message = message + '</div>';
				if (HotelDescription != '') {
					message = message + '<div class="hotel-info__text">' + HotelDescription + '</div>';
				};
				message = message + '</div>';
				message = message + '</div>';
			});
