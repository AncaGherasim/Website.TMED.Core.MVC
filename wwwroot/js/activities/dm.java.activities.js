...
$(document).ready(function () {
    ...
    $('input[name="orderValue"]').click(function () {
        ...
        if (filterParts[1] === '1' || filterParts[2] != '') {
            tempEndIndex = total;
        } if ((ssLoaded / mob.page == 12) || (ssLoaded / desk.page == 24)) {
            if (IsMobileDevice() == true) {
                tempEndIndex = mob.numOfItems * mob.page;
            } else {
                tempEndIndex = desk.numOfItems * desk.page;
            }
        } else {
            tempEndIndex = total;
        }
        if (IsMobileDevice() == true) {
            $(mob.element).html('');
            OpenPage(mob.element, 0, tempEndIndex);
        } else {
            $(desk.element).html('');
            OpenPage(desk.element, 0, tempEndIndex);
        }
    });
    ctyID = $('#cityId').val();
    ...
    //GetCitySSData();
    if (IsMobileDevice() == true) {
        OpenPage(mob.element, startIndex, mob.numOfItems);
    } else {
        OpenPage(desk.element, startIndex, desk.numOfItems);
    }
    getMapPoiData(ctyID);
...
function chooseType(idEl) {
    ...
    if (IsMobileDevice() == true) {
        $(mob.element).html('');
        OpenPage(mob.element, 0, total);
    } else {
        $(desk.element).html('');
        OpenPage(desk.element, 0, total);
    }
...
function chooseFavorite() {
    if ($('#falseshowFav').hasClass('falsechecked_1')) {
        ...
        if (IsMobileDevice() == true) {
            total = mob.numOfItems * mob.page;
        } else {
            total = desk.numOfItems * desk.page;
        }
    } else {
    ...
    if (IsMobileDevice() == true) {
        $(mob.element).html('');
        OpenPage(mob.element, startIndex, total);
    } else {
        $(desk.element).html('');
        OpenPage(desk.element, startIndex, total);
    }
}
...
function buildFilterResults() {
    const marckup = filterResults.map(function ({ name, id }) {
        return `<li>
                    <span>${name}</span>
                    <button type="button" onclick="closeFilterResultItem('${id}')" aria-label="Close">✖</button>
                </li>`
    })
...
function OpenPage(element, startIndex, endIndex) {
    ...
    var allIds = $('#SSIds').val();

    if (allIds != "") {
        var allIdsPages = allIds.split('I')//.join(',').replaceAll(',', ' ').trim().split(' ');
        total = allIdsPages.length;
        $('.spanTotalSS').html(total);
        ...
        options.type = "POST";
        options.contentType = "application/json; charset=utf-8";
        options.data = '{"SSFilter":"' + filter + '", "Ids":"' + allId + '"}',
        options.dataType = "json";
        options.success = function (data) {
            let marckup = "";
            ssLoaded += data.length;
            $('#totalFilterResults').html(data.length + ' total');
